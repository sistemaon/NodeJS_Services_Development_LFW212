...
const BOAT_SERVICE = join(__dirname, 'services/boat-services.7.1.js')
const BRAND_SERVICE = join(__dirname, 'services/brand-services.7.1.js')
...
async function ok (port) {
  const url = `http://localhost:${port}/aggregate/1`
...
async function notFound (port, id) {
  const url = `http://localhost:${port}/aggregate/${id}`
  const res = await get(url)
  assert.equal(
    res.statusCode,
    404,
...
async function badRequest (port) {
  const url = `http://localhost:${port}/aggregate/boat`
  const res = await get(url)
