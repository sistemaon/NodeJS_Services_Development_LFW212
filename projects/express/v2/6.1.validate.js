...
async function start () {
  ...
  await writeFile(join(__dirname, 'models/lab.6.1.model.js'), testingModel())
  ...
  try {
    ...
    await writeFile(join(__dirname, 'models/lab.6.1.model.js'), model())
    sp.kill()
  }
}
...
async function ok (port, id = 1, expect = { brand: 'Chaparral', color: 'red' }) {
  const url = `http://localhost:${port}/lab-6-1/boat/${id}`
...
async function created (port) {
  const url = `http://localhost:${port}/lab-6-1/boat`
  const tx = send(url)
...
async function serverError (port) {
  ...
  const res = await tx(payload)
  console.log("🚀 ~ file: 6.1.validate.js:183 ~ serverError ~ payload:", payload)
