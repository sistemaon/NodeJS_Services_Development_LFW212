...
async function start () {
  ...
  await writeFile(join(__dirname, 'models/lab.5.1.model.js'), testingModel())
  ...
  try {
    ...
    await writeFile(join(__dirname, 'models/lab.5.1.model.js'), model())
    try { sp.kill() } catch {}
  }
}
...
async function ok (port) {
  const url = `http://localhost:${port}/lab-5-1/boat/1`
...
async function notFound (port) {
  ...
  {
    const url = `http://localhost:${port}/lab-5-1/boat/999`
    const res = await get(url)
...
async function serverError (port) {
  const url = `http://localhost:${port}/lab-5-1/boat/${ROUTE_500}`
  const res = await get(url)
