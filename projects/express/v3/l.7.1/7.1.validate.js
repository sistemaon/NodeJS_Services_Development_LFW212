...
async function validate ({ port, boatSrv, brandSrv }, retries = 0) {
  ...
  try {
    if (retries > 3) {
      assert.fail(`Unable to connect to server on port: ${port}/boatbrand`)
    }
    await t(ok(port))
...
async function ok (port) {
  const url = `http://localhost:${port}/boatbrand/1`
...
async function notFound (port, id) {
  const url = `http://localhost:${port}/boatbrand/${id}`
  const res = await get(url)
  assert.equal(
    res.statusCode,
    404,
...
async function badRequest (port) {
  const url = `http://localhost:${port}/boatbrand/boat`
  const res = await get(url)
