...
async function validate ({ port }, retries = 0) {
  ...
  try {
    ...
    await t(notFound(port, 'boat/boat/boat/999'))
...
async function ok (port, id = 1, expect = { brand: 'Chaparral', color: 'red' }) {
  const url = `http://localhost:${port}/boat/boat/boat/${id}`
...
async function noContent (port) {
  const url = `http://localhost:${port}/boat/boat/boat/1`
  ...
  await notFound(port, 'boat/boat/boat/1', 'GET')
}
...
async function serverError (port) {
  const url = `http://localhost:${port}/boat/boat/boat/${ROUTE_500}`
  const tx = send(url, {method: 'delete'})
  const res = await tx()
  assert.equal(
    res.statusCode,
